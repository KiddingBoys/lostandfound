 ...
 * @date 创建时间：2016-5-10 上午12:31:22 
 * @version 1.0 
 * @parameter   
 * @return 
 */
public class ClassifySameOnePresenter{ 
	...
	public ClassifySameOnePresenter(Context context, ILFMsgListView iLSthView) {
 ...
 * @param isRefresh
 */
public void getLFMessage(final boolean isRefresh,String category) {
	if("全部".equals(category)){
		getAllLFMessage(isRefresh);
		return;
	}
	...
	params = "{pageNo:" + page + ",pageCnt:" + number + ",type:all,category:"+category+"}";
	try {
					...
					try {
						...
						if (code == 1) {
							...
							if (resultArray.length() != 0) {
								...
								ToastUtil.toastshow(context, "暂时没有数据");
							}
 ...
 * @param isRefresh
 */
public void getAllLFMessage(final boolean isRefresh) {
	if (isRefresh) {
		page = 0;
	}
